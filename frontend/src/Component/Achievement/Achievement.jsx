...
const AchievementPage = () => {
    ...
    // Media features with image URLs (you would replace these with actual image paths)
    const initialMediaFeatures = [
        {
    ...
    // Function to fetch Open Graph images (optional - if you want to fetch dynamically)
    useEffect(() => {
        // Set initial media features with images
        setMediaFeatures(initialMediaFeatures);
        setLoading(false);
        
        // If you want to dynamically fetch images from URLs, you could use this approach:
        /*
        const fetchMediaImages = async () => {
            try {
                const featuresWithImages = await Promise.all(
                    initialMediaFeatures.map(async (feature) => {
                        try {
                            // This would require a backend API due to CORS restrictions
                            const response = await fetch(`/api/fetch-og-image?url=${encodeURIComponent(feature.link)}`);
                            ...
                            console.error('Error fetching image for:', feature.link, error);
                            return feature; // Return original feature if image fetch fails
                        }
                    })
                );
                setMediaFeatures(featuresWithImages);
            } catch (error) {
                console.error('Error fetching media images:', error);
                setMediaFeatures(initialMediaFeatures); // Fallback to initial data
            } finally {
                setLoading(false);
            }
        };
                                    ...
                                    >
                                        <div className="flex flex-col md:flex-row">
                                            ...
                                            <div className="md:w-2/3 p-6">
                                                ...
                                                <p className="text-gray-600 mb-4">{feature.excerpt}</p>
                                                <a 
                                                    href={feature.link} 
                                                    target="_blank" 
